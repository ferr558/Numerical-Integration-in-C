...
// This function automatically plots a graph of the relative energy as a function of the integration step by comparing it for each algorithm used. If you don't have gnuplot and you don't have a file that has gnuplot code implemented don't use this function.
  void PlotAuto(int algo){
    if(algo==1){
      system("gnuplot --persist 'E_x_v.gnu'");
    }else if(algo==2){
      system(" gnuplot --persist 'EC_x_v.gnu'");
    }else if(algo==3){
      system(" gnuplot --persist 'PC_x_v.gnu'");
    }else if(algo==4){
      system(" gnuplot --persist 'LF_x_v.gnu'");
    }else if(algo==5){
      system(" gnuplot --persist 'V_x_v.gnu'");
    }else if(algo==6){
      system("gnuplot --persist 'Fit.gnu'");
    }
  }
